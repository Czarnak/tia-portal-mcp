...
A separately authorized, PowerShell 7 live-TIA acceptance harness for this contract lives at `scripts/live-test-network-phase2.ps1`. It is never run by any automated test; see the script's own comment-based help for its `Read`/`Preview`/`Apply` modes and required confirmation gates. The read-only I/O-map harness lives at `scripts/live-test-network-io-map.ps1` and is equally never run by automated tests. The pagination harness is [scripts/live-test-hardware-pagination.ps1](https://github.com/Czarnak/tia-portal-mcp/blob/main/scripts/live-test-hardware-pagination.ps1); it is contract-tested but has not been run against live TIA Portal.
